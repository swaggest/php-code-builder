...
class ExportTest extends \PHPUnit_Framework_TestCase
{
     ...
     * @throws \Swaggest\JsonSchema\InvalidValue
     */
    public function testExportMin()
    {
        $schema = \Swaggest\PhpCodeBuilder\Tests\Tmp\SwaggerMin\SwaggerSchema::schema()->exportSchema();
        $schemaData = Schema::export($schema);
        ...
        $result = json_encode($diff->getRearranged(), JSON_PRETTY_PRINT + JSON_UNESCAPED_SLASHES);
        $fileName = realpath(__DIR__ . '/../../../resources') . '/swagger-schema-gen-min.json';
        file_put_contents($fileName, $result);
        exec('git diff ' . $fileName, $out);
        $out = implode("\n", $out);
        $this->assertSame('', $out, "Generated files changed");
